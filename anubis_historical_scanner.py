...
FIXED VERSION - Works with anubis schema database
"""
    ...
    async def _calculate_anubis_scores(self):
        ...
        for wallet, metrics in self.wallet_metrics.items():
            ...
            if developer_tier == "ELITE":
                ...
                alert_priority = "HIGH"  # Alert on likely scams too
            else:
                    ...
                    if launch.get('mint') and launch.get('creator'):
                        try:
                            await conn.execute("""
                                ...
                                (mint_address, creator_wallet, platform, launch_timestamp, launch_signature)
                                VALUES ($1, $2, $3, $4, $5)
                ...
                try:
                    ...
                    await conn.execute("""
                        INSERT INTO anubis.wallet_profiles (
                            ...
                            success_rate,
                            rug_rate,
                            estimated_earnings_sol,
                            largest_success_mcap,
                            average_launches_per_day,
                            peak_launch_hour,
                            first_seen_at,
                            last_seen_at,
                            last_launch_at,
                        ...
                        ON CONFLICT (wallet_address) DO UPDATE SET
                            ...
                            estimated_earnings_sol = EXCLUDED.estimated_earnings_sol,
                            largest_success_mcap = EXCLUDED.largest_success_mcap,
                            average_launches_per_day = EXCLUDED.average_launches_per_day,
                            peak_launch_hour = EXCLUDED.peak_launch_hour,
                    ...
                    """,
                        ...
                        metrics.get('risk_rating', 'MEDIUM'),  # risk_rating maps to risk_level
                        metrics.get('total_launches', 0),
                        metrics.get('successful_launches', 0),
                        0,  # failed_launches - calculate if needed
                        0,  # rugged_launches - calculate if needed
                        metrics.get('success_rate', 0),
                        metrics.get('scam_score', 0),  # scam_score maps to rug_rate
                        metrics.get('estimated_profit', 0),
                        metrics.get('best_mcap_achieved', 0),
                        metrics.get('avg_time_between_launches', 0) / 1440 if metrics.get('avg_time_between_launches') else 0,  # Convert to days
                        metrics.get('preferred_hours', [0])[0] if metrics.get('preferred_hours') else None,
                        metrics.get('first_seen'),
                        metrics.get('last_active'),
                        metrics.get('last_active'),  # Using last_active as last_launch_at
                        (metrics.get('last_active') - metrics.get('first_seen')).days if metrics.get('first_seen') and metrics.get('last_active') else 0,
                        metrics.get('alert_priority', 'STANDARD'),
                        True,  # is_active
                        json.dumps({  # Store extra metrics in tags JSONB
                            'weekend_ratio': metrics.get('weekend_ratio', 0),
                            'asia_session_ratio': metrics.get('asia_session_ratio', 0),
                            'eu_session_ratio': metrics.get('eu_session_ratio', 0),
                            'us_session_ratio': metrics.get('us_session_ratio', 0),
                            'velocity_type': metrics.get('velocity_type'),
                            'time_consistency_score': metrics.get('time_consistency_score', 0),
                            'success_score': metrics.get('success_score', 0),
                            'earnings_score': metrics.get('earnings_score', 0),
                            'avg_seed_amount': float(metrics.get('avg_seed_amount', 0)),
                            'seed_variance': float(metrics.get('seed_variance', 0)),
                            'avg_mcap_achieved': float(metrics.get('avg_mcap_achieved', 0)),
                            'roi': metrics.get('roi', 0)
                        })
                    )
                except Exception as e:
...
async def main():
    ...
    try:
        ...
        # Note: Tables already created by setup_anubis_database.sh
        print("✅ Using existing anubis schema!")
